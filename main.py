...
class MainWindow(Window):
    ...
    def get_fileNum(self, path, day, picCacheCheck, fileCheck, picCheck,
        ...
        # Note：Core data in WeChat 4.0 is saved in msg
        # path_two is used to match this date folder structure like "2022-08" 

        if picCacheCheck:
            path_one = correct_path / 'cache'
            path_two = correct_path / 'cache'
            self.getPathFileNum(now, day, path_one, path_two, file_list,
                                dir_list)
        if fileCheck:
            path_one = correct_path / 'msg/file'
            path_two = correct_path / 'msg/file'
            self.getPathFileNum(now, day, path_one, path_two, file_list,
                                dir_list)
        if picCheck:
            path_one = correct_path / 'msg/attach'
            path_two = correct_path / 'msg/attach'
            self.getPathFileNum(now, day, path_one, path_two, file_list,
                                dir_list)
        if videoCheck:
            path_one = correct_path / 'msg/video'
            path_two = correct_path / 'msg/video'
            self.getPathFileNum(now, day, path_one, path_two, file_list,
                                dir_list)
